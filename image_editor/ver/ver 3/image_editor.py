...
current_frame = None
# 불투명도를 보여주도록 current_frame와 썩을 프레임
overlay = None
# 이미지의 이름
image_name = None
...
def mouse_event(event, x, y, flags, param):
  global current_frame, frames, RGB_color, HSL_color, x0, y0, x1, y1, mouse_flags, mouse_param, image_edited, overlay
  ...
  if event == cv2.EVENT_MOUSEMOVE and flags != cv2.EVENT_FLAG_LBUTTON:
    if brush_type == 6:
      overlay = frames[frame_number-1].copy()
      cv2.rectangle(overlay, (x-size, y-size), (x+size, y+size), (255, 255, 255), -1)

    else:
      overlay = frames[frame_number-1].copy()
      cv2.circle(overlay, (x, y), size, RGB_color, -1)
  ...
  # 마우스가 움직일 때
    ...
    if brush_type == 0:
      ...
      overlay = frames[frame_number-1].copy()
      cv2.line(overlay, (x1, y1), (x, y), RGB_color, size*2)

    elif brush_type == 2:
      overlay = frames[frame_number-1].copy()
      cv2.rectangle(overlay, (x1, y1), (x, y), RGB_color, size*2)

    elif brush_type == 3:
      overlay = frames[frame_number-1].copy()
      cv2.rectangle(overlay, (x1, y1), (x, y), RGB_color, -1)

    elif brush_type == 4:
      overlay = frames[frame_number-1].copy()
      cv2.ellipse(overlay, (int((x+x1)/2), int((y+y1)/2)), (abs(int((x-x1)/2)), abs(int((y-y1)/2))), 0.0, 0.0, 360.0, RGB_color, -1)

    elif brush_type == 5:
      overlay = frames[frame_number-1].copy()
      cv2.ellipse(overlay, (int((x+x1)/2), int((y+y1)/2)), (abs(int((x-x1)/2)), abs(int((y-y1)/2))), 0.0, 0.0, 360.0, RGB_color, size*2)

    elif brush_type == 6:
      roi = background[y-size : y+size, x-size : x+size]
      overlay[y-size : y+size, x-size : x+size] = roi.copy()
...
def new_image():
  global picture, picture_isTaken, background, frames, current_frame, overlay, image_name, cap
